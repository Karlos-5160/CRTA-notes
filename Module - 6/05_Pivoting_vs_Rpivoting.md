...
| :--- | :--- | :--- |
| **Initial Connection** | **Forward / Inbound:** Your Kali reaches *in* to the target host. | **Reverse / Outbound:** The target host reaches *out* to your Kali. |
| **Who is the Server?** | Target runs `sshd`; Kali is the client. | Kali runs `server.py`; target is the client (`client.py`). |
| **Authentication** | Needs SSH credentials or an SSH private key. | Needs zero credentials—just the ability to execute a script. |
...
* If you **have SSH credentials** and port 22 is reachable from your Kali box, use **`ssh -D`**—it is faster, natively supported, and uses SOCKS5[cite: 2, 3].
