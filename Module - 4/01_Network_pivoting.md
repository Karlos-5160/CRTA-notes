...
In enterprise networks and Red Team engagements, high-value identity infrastructure (Domain Controllers, database servers, employee workstations) resides within an isolated internal subnet (`10.10.10.0/24`) with no direct routing to or from the external attacker (`192.168.50.0/24`).
...
When an attacker gains an interactive SSH shell on a dual-homed compromised host:
* **Tooling Absence:** The compromised host is just a server; it does not contain penetration testing toolsets (Impacket, BloodHound/SharpHound, CrackMapExec/NetExec, Responder, wordlists).
* **No GUI Capabilities:** Graphical clients (like `rdesktop` / `xfreerdp`) cannot be displayed or run natively inside a raw terminal shell on the pivot host.
* **OpSec & Artifact Footprint:** Uploading large toolkits, Python runtimes, and binaries onto the target increases detection risk.

**Pivoting solves this** by allowing the attacker to run all tools locally from their Kali machine while transparently relaying the network packets through the pivot host.
...
1. **Comment out `proxy_dns`:**
   ...
   # proxy_dns
   ```
   *(Crucial: Disabling `proxy_dns` prevents tool hangs and timeouts when connecting to direct internal IP addresses).*

2. **Define the Proxy Server at the Bottom:**
   Ensure the `[ProxyList]` section points to your planned local port (e.g., `8090`):
   ```text
   [ProxyList]
...
Dynamic port forwarding instructs OpenSSH to spin up a local SOCKS proxy on your Kali machine and relay requested TCP connections through the remote SSH server (`sshd`).

### Step-by-Step Implementation

#### Step 1: Handling Legacy SSH Host Keys
Older Linux targets (like Metasploitable 2) offer older key types (`ssh-rsa`, `ssh-dss`) that modern OpenSSH disables.
...
* **`-D 8090`**: Binds a SOCKS proxy to local port `8090`.
* **`-f`**: Requests SSH to go to the background just before command execution.
* **`-N`**: Do not execute a remote command/shell (tunnel only).
...
ss -tulpn | grep 8090
```
*(You should see `127.0.0.1:8090` in `LISTEN` state).*
...
With Proxychains configured and the SOCKS tunnel active, prefix any command with `proxychains` to attack internal targets:
...
| :--- | :--- | :--- |
| **No ICMP (`ping`) via SOCKS** | SOCKS4/SOCKS5 works at Layer 4/5 (TCP/UDP); it cannot encapsulate Layer 3 ICMP packets. | Use `proxychains nc -nv <IP> <Port>` or `proxychains nmap -sT -Pn` to verify reachability. |
| **No SYN Stealth Scans (`nmap -sS`)** | SYN scans use raw Layer-3 sockets, which bypass `LD_PRELOAD` hooks. | Always specify `-sT` (TCP Connect) and `-Pn` (No Ping) with `nmap`. |
| **Session Persistence** | The SOCKS proxy terminates instantly if the underlying SSH process drops. | Use `ssh -f -N -D 8090` or keep the session inside `tmux`/`screen`. |